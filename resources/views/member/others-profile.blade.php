    ...
    function drawChart() {
        ...
        $.get( "/ajax/find/downlines?parentUserId="+$userId).done(function( d ) {
            if (d.length == 0) {
                ...
                var rows = [
                    [{v:$userId, f:$nyMame + '<div class="member" style="color:blue; font-style:italic">' + $nyEmail + '</div>'}, '']
                ];
                for(var idx=0; idx<d.length; idx++) {
                    var member = [{v: d[idx].name, f: d[idx].name + '<div class="' + (d[idx].social_id != '' ? 'member' : '') +'" style="color:blue; font-style:italic">' +  d[idx].email +'</div>'}, $userId]
                    rows.push(member);
                }
